...
export function SavedDiagramsDrawer() {
  ...
  const savedDiagramsHasMore = useDiagramStore((state) => state.savedDiagramsHasMore);
  const savedDiagramsTotal = useDiagramStore((state) => state.savedDiagramsTotal);
  const loadSavedDiagrams = useDiagramStore((state) => state.loadSavedDiagrams);
  const loadMoreSavedDiagrams = useDiagramStore((state) => state.loadMoreSavedDiagrams);
  ...
  useEffect(() => {
    ...
    loadSavedDiagrams({ search: savedSearch, scope: savedScope, sort: savedSort, offset: 0 });
    return undefined;
  }, [open, isAuthenticated, savedSearch, savedScope, savedSort, loadSavedDiagrams]);
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-medium text-slate-500 ring-1 ring-slate-200">
                {savedDiagramsTotal > 0 ? `${savedDiagramsTotal} ${t.shown}` : `${savedDiagrams.length} ${t.shown}`}
              </div>
              <button
            ...
            <div className="mt-3 space-y-2">
              {!isSupabaseConfigured ? (
              ...
              ) : loadingSavedDiagrams && savedDiagrams.length === 0 ? (
                ...
                <>
                  ...
                  {savedDiagramsHasMore ? (
                    <button
                      onClick={loadMoreSavedDiagrams}
                      disabled={loadingSavedDiagrams}
                      className="flex w-full items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:cursor-wait disabled:opacity-50"
                    >
                      {loadingSavedDiagrams ? (
                        <>
                          <RefreshCw size={16} className="animate-spin" /> {t.loadingSavedDiagrams}
                        </>
                      ) : (
                        <>
                          <ChevronDown size={16} /> {t.loadMore}
                        </>
                      )}
                    </button>
                  ) : null}
